...
export default function DailyStudyDeck({ cards, onResponse, onRestart }) {
  ...
  function respond(response) {
    ...
    onResponse(currentCard, response);
  ...
  useEffect(() => {
    function handleKeyboard(event) {
      ...
      if (event.key === "ArrowLeft" && currentCard && isFlipped) {
        event.preventDefault();
        respond("review-again");
      }

      if (event.key === "ArrowRight" && currentCard && isFlipped) {
        event.preventDefault();
        respond("got-it");
      }
    }
  ...
  if (!cards.length) {
    return (
      <section className="daily-deck-empty">
        ...
        <Link className="primary-button" to="/training-library">Open the library</Link>
      </section>
    );
  }
  ...
  if (isComplete) {
    return (
      <section className="daily-deck-complete">
        ...
        <p className="eyebrow">Daily practice complete</p>
        <h2>You are ready to get back to service.</h2>
        <p>Facts marked for review will return the next time you practice.</p>
  ...
  return (
    <section className="daily-deck-shell" aria-label="Daily flashcard practice">
          ...
          <article className="daily-flashcard-face daily-flashcard-front">
            <div className="daily-flashcard-media">
              {currentCard.imageUrl ? (
                <img src={currentCard.imageUrl} alt={`${currentCard.title} training photo`} />
              ) : (
      ...
      <div className="daily-response-actions" aria-label="Rate this study card">
        <button type="button" className="daily-response review-again" onClick={() => respond("review-again")} disabled={!isFlipped}>
          <strong>Review Again</strong>
          <span>Bring this card back</span>
        </button>
        <button type="button" className="daily-response got-it" onClick={() => respond("got-it")} disabled={!isFlipped}>
          <strong>Got It Right</strong>
          <span>Continue to the next card</span>
        </button>
      </div>

      <div className="daily-deck-footer">
        <span>Space flips</span>
        <span>Left reviews again</span>
        <span>Right marks correct</span>
        <Link to={`/training-library?open=${currentCard.trainingDocId}`}>Open full training page</Link>
      </div>
    </section>
  );
}
